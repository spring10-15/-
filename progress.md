...
## 2026-03-23 Bilingual + Folded Layout Pass

- Added a persistent `中文 / EN` language toggle to the live shell:
  - menu
  - search
  - table
  - summary
- Added a dedicated `src/i18n.js` layer for:
  - exact-copy translation
  - keyed runtime messages
  - localized table / opponent / item / route names
  - language normalization and persistence defaults
- Wired the game state to remember language across:
  - reset
  - run start
  - summary
  - replay
- Localized the highest-visibility demo surfaces:
  - objective strips
  - extraction review / commit shell
  - room reward previews
  - table pressure shell
  - search presence strip
  - canvas corner labels and scene signage
- Compressed the shell further toward the user's requested one-page feel:
  - search right rail now stays sticky and internally scrolls instead of lengthening the whole page too early
  - deep search tab panels now use internal scroll instead of always pushing the page downward
  - support ledger panel is capped and scrollable inside the tab shell
  - overall shell gaps / paddings / presence-card heights were reduced for denser first-view presentation
- Visual outcome from this pass:
  - the top-right noir scene tag no longer clips in Chinese
  - search feels less like an endlessly stacked report and more like one interactive decision surface with folded sections
  - table mode is substantially more coherent in Chinese, including reward and item surfaces
- Verification this pass:
  ...
  - `node --check src/ai.js`
  - `node --check src/i18n.js`
  - official `develop-web-game` client:
    - `output/web-game-pass39`
    - `output/web-game-pass40`
  - real browser capture sets:
    - `output/pass39-i18n-live/`
    - `output/pass40-layout-check/`
    - `output/pass42-table-zh/`
- No new console or page errors showed up during this pass.
- Remaining known rough edges:
  - `search` is more compact now, but it is still the first place to tighten if user playtest says the page feels too tall
  - the authored demo path is still fixed; bilingual polish was done without changing run structure

## 2026-03-23 Scene-First Sparse UI Pass

- Rebuilt the three demo phases away from the earlier stacked-card presentation and toward a scene-first shell:
  - `search` now uses:
    - a compact top HUD
    - anchored in-scene hotspot buttons
    - one bottom drawer with tabbed content
  - `table` now uses:
    - a lighter inline top HUD
    - a compact left-side dossier
    - a single bottom drawer for players / tools / log
    - a dedicated bottom action bar
  - `summary` now uses:
    - a cinematic result hero
    - a much smaller stat strip
    - one tabbed debrief/details drawer instead of multiple always-open settlement cards
- Preserved the authored demo flow while changing shell composition:
  - no new rooms
  - no new mechanics
  - no new roguelike pool expansion
- Continued the bilingual cleanup:
  - added missing zh keys for the new scene-first shell
  - localized visible summary aftermath text that was still leaking English
  - localized the result stamp in summary (`已结算 / 已扣押`)
- New capture sets for this pass:
...
- Visual read from the pass:
  - `search` now reads much closer to a bar floor with clickable anchors instead of a report page
  - `table` has been reduced to one main scene, one dossier, one drawer, and one action bar
  - `summary` now reads like a result screen first and a ledger second
- Remaining known rough edges after this pass:
  - `search` still has one large objective card that may want one more compression pass if playtest says the eye goes there too early
  - `table` still carries a visible drawer band at the bottom; if it feels too present during playtest, the next step should be reducing its resting height rather than adding more surface detail
  - broader Chinese proofreading should continue during real user play, especially in low-frequency drawer copy

## 2026-03-24 Modal And Table Interaction Pass

- Reworked `search` away from always-open lower panes toward scene hotspots that open focused modal overlays:
  - `Join Game`
  - `Services`
  - `Routes`
  - `Case Folder`
- Added backdrop + modal shell support in the live UI:
  - darkened masked background
  - contained internal scroll
  - per-modal content layouts instead of one shared stacked drawer
- Stopped leaking future authored-path content too early:
  - `Join Game` only shows currently playable rooms
  - search-item reveal actions only target currently visible rooms
  - `Routes` now only exposes currently relevant route choices and focuses on present cost / settlement tradeoffs
- Reworked `table` toward the requested poker-first layout:
  - opponent info is no longer always shown
  - clicking a seat tag opens a right-side player sheet
  - action log is now a hidden side panel toggled from the action rail
  - bottom action area now focuses on playable commands only
- Updated bottom action rail behavior:
  - only legal actions are rendered
  - `Call` / `Raise` now include amounts
  - item use is folded into a hover/focus popover
  - action log toggle moved into the same bottom control rail
- Fixed card readability to feel like real poker instead of prototype data:
  - live cards now render suit glyphs (`♠ ♥ ♣ ♦`)
  - face cards now render as `J / Q / K / A`
  - stage cues and table logs now use pretty card strings too
- Continued Chinese cleanup on visible UI copy:
  - added missing high-frequency keys like `Close`, `Hide`, `Buy-in`, `No read`, `No legal action`
  - normalized several demo-shell labels that were still mixed or awkward
- Real-browser verification for this pass:
  - `output/pass45-ui-check/`
  - `output/pass46-ui-refine/`
  - `output/pass47-viewport-check/`
  - `output/pass48-table-bar-fix/`
  - `output/pass49-table-bar-fix/`
  - `output/pass50-table-actions/`
  - `output/web-game-pass46/`
- Most important verification outcomes:
  - search modals are live and no longer expose deep authored-path rooms early
  - table player sheet opens on seat click
  - table log is hidden by default and opens on demand
  - the action rail is now visible within the live viewport and shows only current legal moves
- No new console or page errors showed up during this pass.
- Remaining known rough edges after this pass:
  - search modals still read a little more like noir drawers than fully centered cinematic popups
  - table overlays are much lighter now, but the top HUD could still compress one more step if the next playtest still feels box-heavy
  - broader Chinese proofreading should continue during hands-on testing, especially in low-frequency summary / reward text

## 2026-03-25 Scene-Center Rebuild

- Unified the live game title around `德扑酒馆：赢了就撤!`:
  - `index.html` title and canvas label
  - visible brand copy in `src/i18n.js`
  - package metadata moved off the old `blacklight-bar` name
- Updated the visual baseline and architecture to a stricter scene-first rule set:
  - one visual center per page
  - no evenly distributed modules
  - motion hierarchy through staggered `transform` / `opacity` animation only
  - scene-specific guidance for 3D targets, particle backdrops, opening motion, and reduced-motion zones
- Rebuilt the active shell composition around a lighter focus model:
  - `menu` remains a centered hero stack
  - `search` now uses:
    - a very small phase chip
    - a centered suspicion meter
    - a minimal right-side resource dock
    - scene hotspots as the primary interaction surface
  - `table` now uses:
    - a smaller phase chip
    - a tighter stat dock
    - smaller side utilities
    - lighter opponent tags that only advertise click-to-inspect instead of exposing extra data up front
- Corrected the biggest modal staging problem:
  - search modals are now centered cinematic overlays instead of reading like bottom drawers
  - the masked background now keeps the scene visible but deprioritized
- Reduced HUD noise further:
  - removed stash / carry from the always-visible search top rail
  - moved off-table cash out of the main table top rail and into the player rail
  - reduced visible toast count to the latest message only
- Continued Chinese cleanup:
  - added missing labels such as `行动中` and `查看`
  - kept visible route copy focused on current extraction cost instead of future-path spoilers
- New verification artifacts for this pass:
  - `output/pass53-single-focus/`
  - `output/pass53-single-focus-refresh/`
- Most useful references from this pass:
  - `output/pass53-single-focus-refresh/search-zh.png`
  - `output/pass53-single-focus/search-routes-zh.png`
  - `output/pass53-single-focus-refresh/table-zh.png`
- Validation completed:
  - `node --check src/main.js`
  - `node --check src/game.js`
  - `node --check src/i18n.js`
  - direct Playwright browser flow through menu -> search -> routes modal -> table
  - no console errors
  - no page errors
- Remaining known rough edges after this pass:
  - `table` is cleaner, but it is still the next highest-value compression target if a playtest says the felt is competing with utility chrome
  - broader Chinese proofreading should continue, especially in low-frequency dossier / summary copy
  - the authored demo path remains intentionally fixed; roguelike variation still stays deferred until the presentation survives playtesting cleanly

## 2026-03-25 Scene Module Refactor

- Reorganized the runtime around scene modules that match the current production mental model:
  - `src/scenes/stashScene.js`
  - `src/scenes/tavernScene.js`
  - `src/scenes/pokerScene.js`
  - `src/scenes/extractionScene.js`
  - `src/scenes/menuScene.js`
- Pulled scene-specific composition out of `src/main.js` so the entry file is now much closer to:
  - input wiring
  - shared helper composition
  - top-level scene routing
  - canvas / UI orchestration
- Made the tavern scene self-own its modal composition:
  - play room modal
  - stash / services modal
  - extraction routes modal
  - folder / notes modal
- Made the poker scene self-own its table UI composition:
  - seat tags
  - dossier / log / player flyouts
  - bottom action rail
  - table-item action generation
- Removed the older `src/scenes/assetsScene.js` compatibility layer and replaced it with the clearer `stashScene.js` naming so the file map matches the actual scene structure.
- Added `scene-code-map.md` at the project root to document:
  - which file owns which scene
  - which files are shared system logic
  - which exports need to stay stable if a single scene is edited in isolation
- Verification completed after the split:
  ...
  - `develop-web-game` Playwright client run against `http://127.0.0.1:4173`
  - direct browser smoke path:
    - menu -> search
    - search hotspot modal open
    - cargo table entry
    - no console or page errors
- Most important structural outcome:
  - you can now revise one scene file without needing to edit the whole UI shell first
  - the scene modules still compose back into one working build through `src/main.js`
- Remaining follow-up worth doing later, but not required for this split:
  - split `styles.css` by scene the same way if scene-local visual iteration becomes the next bottleneck

## 2026-03-25 Style Split And Folder Cleanup

- Continued the scene-based split into the style layer:
  - root `styles.css` is now only an entry file
  - added:
...
- This means future scene-local visual work can mostly stay inside the matching scene CSS file instead of the old single giant stylesheet.
- Cleaned the asset layout so source art and generated output are no longer mixed:
  - moved the three canonical scene-plate images into `assets/scene-plates/`
  - updated runtime references in:
    - `src/main.js`
    - `styles/foundation.css`
    - `visual-direction.md`
    - `presentation-architecture.md`
- Added handoff docs for the new structure:
  - `scene-code-map.md`
  - `styles/README.md`
  - `assets/scene-plates/README.md`
- Intentionally kept the historical `output/` verification artifacts in place for now:
  - many progress and archive notes still reference them
  - they remain useful as visual regression history
  - source scene plates were the important things to remove from `output/`
- Verification completed after the split and cleanup:
  - syntax checks still pass on all active source files
  - browser smoke path still works through:
    - menu
    - search
    - tavern play modal
    - cargo table
  ...
  - no page errors
- New verification artifacts for this pass:
  - `output/pass54-structure-cleanup/`

## 2026-03-25 Root Scene Folder Layout

- Promoted the root-level `scenes/` folder to the single canonical scene workspace.
- The folders you can now pull out and edit independently are:
  - `scenes/stash/`
  - `scenes/tavern/`
  - `scenes/poker/`
  - `scenes/extraction/`
  - `scenes/menu/` for the title/start scene
- Each scene folder now contains:
  - `index.js`
  - `style.css`
  - `README.md`
- Updated `src/main.js` to import directly from the root `scenes/` folders.
- Removed the duplicate `src/scenes/` source layer so the project no longer has two competing scene-code locations.
- Updated the style entry so scene-local CSS now loads from:
  - `scenes/menu/style.css`
  - `scenes/stash/style.css`
  - `scenes/tavern/style.css`
  - `scenes/poker/style.css`
  - `scenes/extraction/style.css`
- Added root-level handoff docs:
  - `scenes/README.md`
  - per-scene `README.md` files
- Verification completed:
  ...
  - no page errors
- New verification artifacts for this pass:
  - `output/pass55-scene-root-map/`

## 2026-03-25 Stash-First Scene Pass

- Turned the stash / hideout UI from a modal-only slice into a real standalone scene:
  - added `renderStashScene(state, helpers)` in `scenes/stash/index.js`
  - the first `search`-mode landing page after `start-run` is now `stash`, not the tavern floor
- Added a local search-scene switch in `src/main.js`:
  - new UI state `activeSearchScene`
  - entering `search` after `start-run` defaults to `stash`
  - later returns to `search` after a cleared table default to `tavern`
- Added explicit stash/tavern scene pills so the two search-space scenes can be iterated separately without changing the shared run logic.
- Kept the stash scene focused on the requested first-scene responsibilities:
  - personal assets
  - stash / deposits
  - services and tools
  - case folder / notes
  - a clean hand-off into the tavern floor
- Moved the tavern-side “stash” hotspot away from being a services modal and turned it into a return path back to the desk.
- Added real stash scene CSS in `scenes/stash/style.css`.
- Added shared search-scene nav styling in `styles/shared-scene.css`.
- Updated handoff docs:
  - `scenes/stash/README.md`
  - `scenes/README.md`
  - `scene-code-map.md`
- Fixed a real plan inconsistency while doing this:
  - `development-plan.md` still referenced deleted `src/scenes/*.js` paths
  - updated those references to the canonical root `scenes/` folders
- Verification completed for this pass:
  - `node --check src/main.js`
  - `node --check scenes/stash/index.js`
  - `node --check scenes/tavern/index.js`
  - `node --check src/i18n.js`
  - official web-game Playwright client confirms `searchScene: "stash"` after `start-run`
  - browser smoke confirms:
    - menu -> start run -> stash scene
    - stash services modal opens
    - stash -> tavern scene switch works
- New verification artifacts for this pass:
  - `output/pass56-stash-first/`
  - `output/pass57-stash-ui/`
  - `output/pass58-stash-refine/`
  - `output/pass59-stash-to-tavern/`

## 2026-03-25 Stash Art Separation Pass

- Replaced the stash scene's reused tavern art with the new dedicated hideout scene plate:
  - `assets/scene-plates/藏匿点场景.png`
- Replaced the personal-assets / services modal background with the new dedicated asset-table art:
  - `assets/scene-plates/个人资产弹窗.png`
- Updated `src/main.js` so search-mode illustration routing now distinguishes:
  - `stash`
  - `tavern`
  instead of treating all search-mode screens as the tavern plate
- Updated `styles/foundation.css` so the page-shell background also respects `data-search-scene="stash"` instead of continuing to show the tavern shell behind the canvas
- Added stash-modal-specific styling in `scenes/stash/style.css` so the services / personal-assets modal reads like a separate asset table instead of a generic dark dialog
- Updated `assets/scene-plates/README.md` to reflect the expanded scene-art set
- Verification completed for this pass:
  - `node --check src/main.js`
  - official web-game Playwright client confirms `searchScene: "stash"` while using the new stash art
  - browser smoke confirms:
    - stash scene uses the hideout plate
    - services modal uses the personal-assets plate
    - tavern scene still uses the tavern plate
- New verification artifacts for this pass:
  - `output/pass61-stash-art/`
  - `output/pass62-stash-services-art/`

## 2026-03-25 Folder Modal Art Pass

- Wired the dedicated dossier art into the stash scene's folder modal:
  - `assets/scene-plates/案件夹弹窗.png`
- The stash scene now uses three distinct visual layers:
  - hideout scene plate
  - personal-assets/services modal plate
  - folder/dossier modal plate
- This removes the last major visual overlap inside the first scene cluster:
  - stash is no longer tavern
  - services is no longer dossier
  - folder is no longer a generic dark overlay
- Verification completed for this pass:
  - official web-game Playwright client still passes after the style change
  - browser smoke confirms the folder modal opens with the dedicated dossier art
- New verification artifacts for this pass:
  - `output/pass63-folder-art/`
  - `output/pass64-folder-modal/`

## 2026-03-26 Menu Title Art Hookup

- Replaced the old menu overlay panel with the new authored title-screen plate:
  - `assets/scene-plates/menu-title-bg.png`
- The menu scene now uses three transparent hotspot buttons aligned to the painted UI:
  - `start-run`
  - `load-run`
  - `exit-game`
- Added a browser-side exit action for the title screen:
  - tries to leave fullscreen
  - attempts to close the window
  - falls back to `about:blank` if the browser refuses to close
- Updated menu background references so both the CSS shell and canvas art use the title plate.
- Verification completed for this pass:
  - `node --check scenes/menu/index.js`
  - `node --check src/main.js`
  - `node --check src/i18n.js`
  - official web-game Playwright client confirms clicking `#start-btn` leaves `mode: "menu"` and enters `mode: "search"`
  - browser screenshot confirms the title art renders cleanly without the old menu text overlay
- New verification artifacts for this pass:
  - `output/menu-title-preview/`
  - `output/menu-hotspots/`

## 2026-03-26 Viewport Cleanup And Card Asset Pass

- Reworked the four playable scene clusters so they stay on the first screen without page scrolling:
  - stash
  - tavern
  - poker table
  - extraction summary
- Added a final override layer in `styles/scene-reset.css` to remove heavy stacked framing, reduce repeated panel chrome, and keep modal content inside the viewport.
- Converted the poker cards from CSS-rendered faces to the supplied transparent PNG deck:
  - copied face and back assets into `assets/cards/`
  - updated `src/main.js` card rendering to use those images in both visible and hidden states
- Simplified the stash and tavern scenes into lighter top HUD + center focus + bottom action layouts instead of multiple framed windows.
- Converted in-scene secondary tools to transparent overlay modals instead of fixed side panels:
  - search services / folder
  - tavern play / routes / folder
  - table dossier / log / player inspection
- Simplified the poker table specifically to reduce overlap:
  - seat tags now show only names unless the player is acting or folded
  - action buttons were separated from the player cards
  - dossier and log were demoted into lighter secondary controls
  - handoff and toast feedback were reduced to quieter overlays
- Simplified the extraction summary into a single centered settlement sheet instead of multiple drawers.
- Added the missing Chinese localization for:
  - `No payout costs`
- Verification completed for this pass:
  - `node --check scenes/stash/index.js`
  - `node --check scenes/tavern/index.js`
  ...
  - `node --check src/main.js`
  - `node --check src/i18n.js`
  - browser smoke confirms `hasPageOverflow: false` and `hasUiOverflow: false` for:
    - menu
    - stash
    - tavern
    - tavern play modal
    ...
    - poker dossier modal
- New verification artifacts for this pass:
  - `output/pass67-ui-review-clean/`
  - `output/pass68-ui-review-final/`

## 2026-03-26 Full Flow Verification Pass

- Added a logic-level regression harness to exhaustively walk the game-state machine:
  - `scripts/verify_full_game_flow.mjs`
- Added a browser-level verification harness to drive the real UI through the major scene paths and save screenshots:
  - `scripts/verify_browser_flows.mjs`
- Fixed a real progression bug where `Sleeve Clip` logic existed but the item could not be reached during normal progression:
  - moved `sleeve-clip` into the stage-2 search stock in `src/data.js`
- Restored the Mirror Hall collateral selector in the tavern play modal after the UI simplification pass:
  - players can once again choose a carried valuable before entering `mirror-hall`
- Tightened the extraction route modal cards so route selection stays lighter and fits the one-screen layout more reliably.
- Verification completed for this pass:
  - `node --check src/data.js`
  - `node --check scenes/tavern/index.js`
  - `node --check scripts/verify_full_game_flow.mjs`
  - `node --check scripts/verify_browser_flows.mjs`
  - `node scripts/verify_full_game_flow.mjs`
  - `node scripts/verify_browser_flows.mjs`
  - official web-game Playwright client smoke still passes after the flow fixes
- Logic verification coverage for this pass includes:
  - menu start and load branches
  - stash services, intel purchase, item purchase, sell, and heat reduction
  - search consumable branches
  - table entry gating, fold, reward payout, and failure resolution
  - extraction route branches
  - Mirror Hall collateral return and collateral loss
  - wallet salvage and reward-blocked inventory handling
  - Sleeve Clip, Marked Lens, and Signal Lighter item flows
- Browser verification coverage for this pass includes:
  - menu, start, load, and exit
  - stash main scene, services modal, and folder modal
  - tavern main scene, routes modal, folder modal, and table-entry modal
  - cargo table and mirror table
  - dossier, log, and player-inspection overlays
  - success summary and failure summary
- New verification artifacts for this pass:
  - `output/full-game-flow-report.json`
  - `output/browser-full-coverage/`
  - `output/pass70-web-client-smoke/`

## 2026-03-26 Menu Single-Layer Presentation Pass

- Flattened the title screen so it no longer renders as a framed picture-in-picture card over a second copy of the same art.
- The menu scene now behaves as a transparent hotspot overlay aligned directly to the main scene canvas.
- Disabled the duplicate menu-specific shell background layer and removed menu-mode shell chrome so the first screen reads as one continuous image.
- Removed menu-mode UI padding so the three hotspot buttons stay aligned with the authored title-screen art.
- Verification completed for this pass:
  - `node --check scenes/menu/index.js`
  - `node --check src/main.js`
  - official web-game Playwright client confirms the single-layer title screen renders and `#start-btn` still enters `mode: "search"`
- New verification artifacts for this pass:
  - `output/pass71-menu-single-layer/`
  - `output/pass72-menu-start-after-flatten/`

## 2026-03-26 Menu Full-Bleed Correction Pass

- Removed the remaining menu scene "inner stage" behavior that was still making the title art look like a picture-in-picture composition on large screens.
- Menu mode now hides `#game-canvas` entirely and renders the title screen as a single full-bleed background layer with only hotspot buttons on top.
- Removed the remaining menu shell overlay and menu-mode vignette so the start screen reads as one uninterrupted authored image.
- Kept the menu interaction model unchanged:
  - start
  - load
  - exit
- Verification completed for this pass:
  - `node --check src/main.js`
  - official web-game Playwright client confirms the title screen renders as a single layer
  - official web-game Playwright client confirms `#start-btn` still enters `mode: "search"`
- New verification artifacts for this pass:
  - `output/pass73-menu-true-fullbleed/`
  - `output/pass74-menu-fullbleed-start/`

## 2026-03-26 Flow Polish Pass

- Added `Esc` support for live overlays so the player can dismiss:
  - search-scene modals
  - table overlays
  - extraction review state
- Updated the search-scene primary CTA logic so it points at the actual next step instead of always repeating the same verb:
  - stash now jumps straight into route review when the run is already carrying pressure
  - tavern now promotes route review when extraction pressure is active
  - tavern still keeps `Join Game` visible even when routes become the primary action
- Extended the scene-switch handling so a button can both:
  - move between stash and tavern
  - open the intended modal immediately
- Updated the browser verification harness to understand the new route-first flow instead of assuming tavern always returns to a `Join Game` primary button.
- Verification completed for this pass:
  - `node --check src/main.js`
  - `node --check scenes/stash/index.js`
  - `node --check scenes/tavern/index.js`
  ...
  - `node scripts/verify_full_game_flow.mjs`
  - `node scripts/verify_browser_flows.mjs`
  - targeted browser check confirms:
    - `Esc` closes stash overlays
    - stash route-first primary opens routes immediately
    - tavern route-first primary promotes routes while `Join Game` stays available
- New verification artifacts for this pass:
  - `output/pass75-flow-polish/`
  - `output/browser-full-coverage/`

## 2026-03-26 Scene Overlay And Floor-Lock Pass

- Reworked the stash and tavern scene interactions so the main search-phase buttons no longer sit in a rigid shared dock.
- Moved stash hotspots onto scene-specific props:
  - `Inventory And Notes` now rides the case-folder corner
  - `Services And Tools` now sits over the desk kit area
  - `Step Onto The Floor` now anchors to the upstairs / suitcase side instead of colliding with the old button row
- Moved tavern hotspots onto scene-specific props:
  - `Join Game` now points at the back-room poker table
  - `Services And Tools` now lives on the bottle shelf
  - `Review Routes` now sits over the map / exit-planning side of the bar
  - `Inventory And Notes` stays on the dossier side instead of sharing the hero CTA lane
- Removed the extra mid-screen instructional objective cards from stash and tavern so the scenes read more like authored environments and less like guided dashboards.
- Fixed the broken half-height modal behavior by letting search-scene overlays use the global full-viewport modal system again instead of scene-local overrides that were forcing smaller dialog boxes.
- Kept the stash services and folder overlays attached to the authored popup art in:
  - `assets/scene-plates/个人资产弹窗.png`
  - `assets/scene-plates/案件夹弹窗.png`
- Kept the new floor-commit rule live:
  - once the player enters the tavern floor, hideout-only cash parking stays locked
  - stash-to-tavern flow is now one-way for the rest of the run
- Added matte / backing shells behind the transparent poker card PNGs so player and board cards read cleanly against both table scenes.
- Hardened the verification harnesses:
  - browser flow uses a deterministic settle helper for rigged final-hand checks
  - logic flow now forces the illegal-action scenario into a truly illegal state instead of relying on a random opening action
- Verification completed for this pass:
  - `node --check scenes/stash/index.js`
  - `node --check scenes/tavern/index.js`
  - `node --check scripts/verify_browser_flows.mjs`
...
- Verification results for this pass:
  - browser coverage report is clean with `errors: []`
  - logic coverage report passes `25 / 25`
- Refreshed verification artifacts for this pass:
  - `output/browser-full-coverage/`
  - `output/full-game-flow-report.json`

## 2026-03-26 Prop-Locked Hotspots And Route Intel Pass

- Re-anchored the authored scene hotspots so they now stay tied to the intended props instead of swapping lanes based on flow priority:
  - stash `Personal Assets` now sits on the money case
  - stash `Inventory And Notes` now sits on the notebook / case file area
  - stash `Cargo Table` now sits on the desk map instead of colliding with the notebook hotspot
  - stash `Services And Tools` now stays on the chips / gun-side kit area
  - tavern `Cargo Table` now stays on the back poker table
  - tavern `Inventory And Notes` now stays on the front desk / bartender side
  - tavern `Review Routes` now stays on the doorway lane
  - tavern `Services And Tools` now stays on the safe / bottle shelf side
- Removed the tavern hotspot role-swapping behavior so extraction priority no longer teleports `Join Game` away from the poker table or moves route review onto the wrong prop.
- Tightened route visibility so exit lines now come from floor pressure and intel instead of hideout-only prep:
  - public exit now needs live floor intel or a completed room
  - fixed whisper lines now need whispers, opponent intel, or a completed cargo room
  - emergency exits now wake up from floor pressure, exposed goods, or room outcomes
- Kept the one-way floor rule intact:
  - after `enter-floor`, the hideout banking shortcut stays closed
  - dead-end states with no affordable room and no live route now fail the run instead of pretending the search can continue
- Restored the inventory loop inside the search presentation:
  - search-phase tools can be bought, sold, and used from the authored overlays
  - valuables and tools can both be sold for cash before or between rooms
- Finished the poker control pass:
  - player action row is now `Fold / Check / Call / All-in`
  - fixed the action-key mismatch that made `All-in` look legal but fail dispatch
  - fixed exact-stack calls so matching the current bet no longer forces an incorrect fold
  - kept the warm white / yellow card shells under the transparent PNG card art
- Verification completed for this pass:
  - `node --check src/game.js`
  - `node --check src/ai.js`
  - `node --check src/main.js`
  ...
  - `node scripts/verify_browser_flows.mjs`
- Verification results for this pass:
  - logic coverage passes `28 / 28`
  - browser coverage report remains clean with `errors: []`
- Refreshed verification artifacts for this pass:
  - `output/browser-full-coverage/stash.png`
  - `output/browser-full-coverage/tavern.png`
  - `output/browser-full-coverage/stash-tools.png`
  - `output/browser-full-coverage/mirror-table.png`
  - `output/full-game-flow-report.json`

## 2026-03-26 Table Readability Polish Pass

- Added the street-progress rail back into the table scene so the Hold'em hand now reads as a visible multi-street flow instead of five anonymous board slots.
- Hid the repeated per-card street labels under the board and let the new reveal rail carry that information more cleanly.
- Added a lighter command tray under the player hand so the four action buttons stay readable over the painted background while still keeping the scene sparse.
- Kept disabled actions visible instead of letting them disappear into the art:
  - `Fold`
  - `Check`
  - `Call`
  - `All-in`
- Nudged the authored hotspots again for polish:
  - stash `Inventory And Notes` sits closer to the desk notebook lane
  - tavern `Review Routes` and `Inventory And Notes` sit more cleanly in the right-side interaction cluster
- Verification completed for this pass:
  - `node --check src/main.js`
  - `node --check scenes/poker/index.js`
  - `node scripts/verify_full_game_flow.mjs`
  - `node scripts/verify_browser_flows.mjs`
- Verification results for this pass:
  - logic coverage still passes `28 / 28`
  - browser coverage still reports `errors: []`

## 2026-03-27 Table Pace And Overlay Stabilization Pass

- Locked the language toggle to the homepage only:
  - menu still shows `中文 / EN`
  - stash, tavern, table, extraction, and summary no longer repeat that control
- Stabilized authored search hotspots:
  - removed the last hover / float drift so stash and tavern buttons no longer feel like they jump while searching
  - moved the hideout center hotspot upward onto the map lane and renamed it from the room-specific copy to `Hit the Tavern`
- Deepened modal separation:
  - darkened the fullscreen matte behind stash / tavern / table overlays
  - strengthened the modal panel fill so overlay content reads cleanly against the painted backgrounds
- Reworked poker table pacing into a staged flow instead of instant resolution:
  - AI now acts one move at a time with visible pauses
  - flop / turn / river reveals each get their own beat
  - hand conclusions linger before the next hand or room exit
  - final hands now stay visible long enough for the player to read the board and result
- Surfaced per-opponent action memory:
  - left-front and right-front opponent tags now stay visible on the table
  - each tag shows the opponent's latest move such as `Check`, `Call`, `Bet`, or `All-in`
- Expanded the hand-close recap so the player can read why the hand ended:
  - board line
  - river card
  - player's revealed hand
  - revealed opponent hands when the pot reaches showdown
- Removed the remaining poker-layer entrance animations that were leaving seat tags, felt panels, and the player rail visually stuck at `opacity: 0` during automated browser runs
- Verification completed for this pass:
  - `node --check src/game.js`
  - `node --check src/main.js`
  - `node --check scenes/poker/index.js`
...
- Verification results for this pass:
  - logic coverage still passes `28 / 28`
  - browser coverage reports `errors: []`
- Refreshed verification artifacts for this pass:
  - `output/browser-full-coverage/stash.png`
  - `output/browser-full-coverage/stash-tools.png`
  - `output/browser-full-coverage/tavern.png`
  - `output/browser-full-coverage/cargo-table.png`
  - `output/browser-full-coverage/mirror-table.png`
  - `output/browser-full-coverage/cargo-log.png`

## 2026-03-30 UI Polish Update

- Simplified the hideout scene:
  - removed the hideout-only services/tools hotspot
  - reduced the top-left scene copy to a single scene name
  - moved the tavern entry hotspot onto the map area
  - repacked the folder modal into inventory, tavern battle records, and opponent intel
- Simplified the tavern scene:
  - reduced the top-left scene copy to a single scene name
  - widened and stabilized the route / inventory hotspots so they no longer feel jumpy or cramped
- Simplified the poker table scene:
  - removed the extra dossier button and old flow strip from the main layout
  - kept one hand-log button plus a compact live hand-log preview
  - compressed the user area into hole cards + one concise status strip
  - added tooltip titles to inventory cards and table-item buttons
- Darkened modal presentation to a 75% mask overlay.
- Added `run.roomResults` so stash records can show completed tavern results.
- Updated browser flow coverage to match the new hideout/tavern layout.
- Validation:
  - `node scripts/verify_browser_flows.mjs` -> `errors: []`
  - `node scripts/verify_full_game_flow.mjs` -> `28 / 28` passed
- Follow-up polish still worth doing:
  - tighten hideout hotspot placement further to exact object centers
  - reduce the remaining player status bar visual weight on the poker table
  - replace browser default tooltip styling with a custom in-world tooltip treatment if desired

## 2026-03-30 BGM Integration Update

- Wired scene-based BGM playback from `assets/audio/bgm/`.
- Added one shared audio manager in `src/main.js` with:
  - first-interaction unlock
  - scene-to-scene fade transitions
  - per-scene target volumes
  - summary success / failure split tracks
- Current scene mapping:
  - menu -> `menu-theme.ogg`
  - hideout -> `stash-loop.ogg`
  - tavern floor -> `tavern-floor.ogg`
  - poker table -> `table-pressure.ogg`
  - summary success -> `summary-success.ogg`
  - summary failure -> `summary-failure.ogg`
- Validation:
  - `node scripts/verify_browser_flows.mjs` -> `errors: []`
