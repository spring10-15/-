...
func run() -> void:
	...
	check(world.request_action(world.door_target), "Door interaction opens departure panel")
	check(world.run_panel.visible and world.current_room == "stash", "Door shows cost before travel")
	...
	check(world.request_action(back), "Return door opens extraction panel")
	check(world.run_confirm.disabled, "Unknown exit cannot be used")
	world.close_run_panel()
	await aim(Vector3(8.0, 0.02, 2.55), world.exit_notice.global_position)
	check(world.request_action(world.exit_notice), "Exit notice is reachable through real ray")
	await aim(Vector3(8.0, 0.02, 1.65), back.global_position)
	world.request_action(back)
	world.run_confirm.pressed.emit()
