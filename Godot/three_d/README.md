...
**已接通带钱出发、首桌买入、离桌返还、寻找出口和付费撤离；资金仅在本次运行中保留，关闭程序不保存。** 酒馆还是灰盒；Blender 的三张图像贴图已保留，程序化材质暂用 PBR 基础色，烘焙、人物、音效、存档、完整搜索与库存、其它撤离路线和正式美术仍待制作。此版不是最终画质样板。
...
资金测试 640 项，界面整局测试 58 项。具体口径见项目内 docs/3d-production/run-migration.md。

## 双桌流程验证

运行脚本 res://three_d/tests/two_tables.gd（参数 -- --test），共 27 项检查。增加 --capture 并使用 Metal 图形模式，可生成 output/3d/ledger-table.png。

账房地窖目前复用粗模空间布局；已接入原版买入、盲注、对手和解锁条件，道具额外热度与物品奖励尚未接入。
